...
public static class B2CInvitationLandingEndpoint
{
    internal static RouteHandlerBuilder MapB2CInvitationLandingEndpoint(this IEndpointRouteBuilder endpoints)
    {
        return endpoints
            .MapGet("/api/public/invitation/{token}", async (
                string token,
                HttpContext context,
                AnonymousInvitationService invitationService,
                IOptions<AuthenticationOptions> authOptions,
                IOptions<OriginOptions> originOptions,
                ILogger<IInvitationService> logger) =>
            {
                logger.LogInformation("Invitation landing page requested for token: {Token}", token);
                ...
                var b2cOptions = authOptions.Value.AzureAdB2C;
                if (b2cOptions == null)
                {
                ...
                // Get the tenant name
                // Use the tenant ID as the tenant name for now
                ...
                logger.LogInformation("Sign-up URL: {SignUpUrl}", signUpUrl);
                
                return Results.Content(html, "text/html");
            })
            .WithName("B2CInvitationLanding")
            .WithSummary("Display invitation acceptance landing page")
            .WithDescription("Shows a styled landing page for users to accept their invitation")
            .AllowAnonymous()
